                ...
                <div class="my-4 flex gap-4">
                    <div class="w-1/2 _mr-3">
                        <div class="w-full md:w-14rem mb-4">
                            <label for="month-select" class="block text-900 font-medium mb-2">Mês</label>
                            <Dropdown
                                id="month-select"
                        ...
                        <label for="month-select" class="block text-900 font-medium mb-2">Início</label>

                        <DatePicker v-model="startDate" placeholder="Data início" showIcon dateFormat="dd/mm/yy" />
                    </div>
                    <div class="w-1/2 _mr-3">
                        <div class="w-full md:w-14rem mb-4">
                            <label for="month-select" class="block text-900 font-medium mb-2">Ano</label>
                            <Dropdown
                                id="year-select"
                        ...
                        <label for="month-select" class="block text-900 font-medium mb-2">Fim</label>

                        <DatePicker v-model="endDate" placeholder="Data fim" showIcon dateFormat="dd/mm/yy" />
                    </div>
                </div>
                <div>
                    <label for="month-select" class="block text-900 font-medium mb-2">Categoria</label>
                    <Dropdown
                        ...
                        class="w-full _mr-3 mb-3"
                    />
                    <label for="month-select" class="block text-900 font-medium mb-2">Contato</label>
                    ...
                    <div class="flex gap-4">
                        ...
                        />
                        <Button label="Carregar Transações" @click="load_data" class="w-1/2 p-button-raised p-button-success" />
                    </div>
                </div>
            </SuperCard>
        </div>
...
<script setup lang="ts">
    ...
    import { executeQuery, executeQueryRun, formatCurrency, formatDateForSQL, formatDateBR } from '@/utils/db';
    ...
    async function load_data() {
        ...
        contacts.value = await load_contacts();
        noticeDetails.value = [
    ...
    const load_contacts = async () => {
        let res = await executeQuery('select * from contacts order by name');
        res = res.map((contact) => ({
            key: contact.id,
            value: contact.name + (contact.type === 1 ? ' (Pessoa)' : ' (Empresa)'),
        }));
    ...
    const load_transactions = async () => {
        ...
        if (selectedContact.value) {
            sql += `
          AND ft.related_id = ${selectedContact.value} `;
        }
    ...
    const load_totais = async () => {
        ...
        const contact_id = selectedContact.value ?? null;
